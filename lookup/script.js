...
async function loadWords() {
  try {
    const response = await fetch('bookworm_with_bugs.txt');
    ...
    document.getElementById('wordList').innerHTML =
        '<div class="word-item">Error loading dictionary file</div>';
...
function filterWords(filterText) {
  if (filterText === '') {
    ...
    filteredWords = allWords.filter(
        word => word.toUpperCase().startsWith(filterText.toUpperCase()));
  }
...
function setupEventListeners() {
  ...
  filterInput.addEventListener('paste', function(e) {
    ...
    const newValue = currentValue.substring(0, start) + cleanPaste +
